...
\begin{itemize}
    ...
    \end{enumerate}
    \vspace{.2cm}
    \item<2-> We explored \textbf{stochastic parameterization} : {stochastic advection by Lie transport} (SALT)
    \item<2-> Why {SALT}? 
    \begin{itemize}
        \item<2-> preserves physical structure $\rightarrow$ reliable long simulations
        \item<2-> better forecast skills $\rightarrow$ uncertainty due to unresolved transport processes can be quantified
        \item<2-> conducive to data-driven approaches $\rightarrow$ can be estimated from observations/satellite data
    \end{itemize}
