...
\begin{itemize}
    ...
    \begin{itemize}
        \item<4-> 2D model: solves for velocity, temperature, and pressure
    \end{itemize}
    \vspace{0.1cm}
    \item<5-> \textcolor{orange}{Stochastic:} has stochastic/random terms (think of Brownian motion)
    \vspace{0.1cm}
    \item<6-> \textcolor{purple}{Structure-preserving:} preserves underlying geometric and physical structure
    \vspace{0.1cm}
    \item<7-> \textcolor{teal}{Development:} modeling, analysis, and \textcolor{teal}{simulation}
\end{itemize} 
\vspace{5mm}
\onslide<8>{\large \textbf{We are solving stochastic PDEs which model ocean-atmosphere interactions!}}
\end{frame}
...
\begin{itemize}
    ...
    \begin{enumerate}
        ...
        \item<2-> Stochastic
    \end{enumerate}
    \item<3-> We explored \textbf{stochastic parameterization} : {stochastic advection by Lie transport}\footfullcite{holmVariationalPrinciplesStochastic2015} (SALT)
    \vspace{2mm}
    \item<4-> Why \textbf{SALT}? 
    \begin{itemize}
        \item<4-> Structure preservation
        \item<4-> Better uncertainty quantification skills
        \item<4-> Data-driven approach
    \end{itemize}
\end{itemize}
\end{frame}
...
\begin{enumerate}
    ...
    \only<2> {\st{Compare the stochastic model results with other existing models}}
\end{enumerate}    
\end{frame}
\section{The climate model}
\begin{frame}{Model equations}
  \onslide<1-> 2D climate model equations\footfullcite{crisanImplementationHasselmannsParadigm2023}:
\begin{columns}[T] % align columns
\begin{column}{.65\textwidth}
% \begin{itemize}
    % \item<1-> 2D climate model equations\footfullcite{crisanImplementationHasselmannsParadigm2023}:
    \begin{footnotesize}
    \begin{align*}
...
\end{figure}
$\mathbf{u}:$ velocity, $\theta:$ temperature $p:$ pressure\\ $Re:$ Reynolds number \\ $Pe:$ P\'{e}clet number\\ $Ro:$ Rossby number \\
$\sigma, \gamma:$ coupling coefficients}
\end{column}%
\end{columns}
\end{frame}
\begin{frame}
